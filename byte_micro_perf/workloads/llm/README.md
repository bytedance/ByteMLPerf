...
## head_rms_norm
Given hidden_states (**[num_tokens, (q_head_num + 2 * kv_head_num), head_dim]**, bfloat16), for each token, rms_norm on head_dim dim on specified heads [head_start, head_start + head_num)

- (in) hidden_states:
    - [num_tokens, (q_head_num + 2 * kv_head_num), head_dim]
    - bfloat16
    - support stride on **(q_head_num + 2 * kv_head_num)**, which means only some heads will be rms_normed.
- (in) weight:
    - [head_dim]
    - bfloat16 or float32
---
