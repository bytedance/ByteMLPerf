...
class BackendMLU(Backend):
    ...
    def get_backend_properties(self):
        self.memory_limit = int(
            torch.mlu.get_device_properties(0).total_memory / (1024**3)
        )
    ...
    def host2device(self):
        self.op = Host2DeviceOp(torch.device("mlu"))
    ...
    def build_tensor(self, input_shapes, dtype):
        ...
        for _ in range(max_data_cnt):
            ...
            if hasattr(self.op, "custom_create_tensors"):
                input_tensors = self.op.custom_create_tensors(input_shapes, torch_dtype, "mlu")
                input_tensors_list.append(input_tensors)
            # default: all input tensors have same dtype
            else:
                if torch_dtype in [torch.int8, torch.int32]:
                    input_tensors = [
                        torch.randint(-3, 3, size=shape, dtype=torch_dtype, device="mlu")
                        for shape in input_shapes
                    ]
                else:
                    input_tensors = [
                        torch.randn(shape, dtype=torch_dtype, device="mlu")
                        for shape in input_shapes
                    ]
                input_tensors_list.append(input_tensors)
    ...
    def setup_2d_group(self):
        ...
        torch.mlu.set_device(self.rank)
