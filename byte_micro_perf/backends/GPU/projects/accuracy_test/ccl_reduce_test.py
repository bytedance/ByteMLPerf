    ...
    try:
        ...
        for exponent in range(default_exponent, default_exponent + 13):
            ...
            if rank == 0:
                ...
                print(f"fp64:\t\t{fp64_golden_value.flatten()[0].item()}\t{fp64_exponent}\t{bin(fp64_mantissa)} ({dtype_mapping[torch.float64].mantissa_bits}) (e^{true_fp64_exponent}={2 ** true_fp64_exponent}) (e^{true_fp64_fraction}={2 ** true_fp64_fraction})")
                print(f"fp32:\t\t{fp32_golden_value.flatten()[0].item()}\t{fp32_exponent}\t{bin(fp32_mantissa)} ({dtype_mapping[torch.float32].mantissa_bits}) (e^{true_fp32_exponent}={2 ** true_fp32_exponent}) (e^{true_fp32_fraction}={2 ** true_fp32_fraction})")
                print(f"cast bf16:\t{cast_bf16_golden_value.flatten()[0].item()}\t{cast_bf16_exponent}\t{bin(cast_bf16_mantissa)} ({dtype_mapping[torch.bfloat16].mantissa_bits}) (e^{true_cast_bf16_exponent}={2 ** true_cast_bf16_exponent}) (e^{true_cast_bf16_fraction}={2 ** true_cast_bf16_fraction})")
                print(f"bf16:\t\t{bf16_golden_value.flatten()[0].item()}\t{bf16_exponent}\t{bin(bf16_mantissa)} ({dtype_mapping[torch.bfloat16].mantissa_bits}) (e^{true_bf16_exponent}={2 ** true_bf16_exponent}) (e^{true_bf16_fraction}={2 ** true_bf16_fraction})")
                print(f"nccl bf16:\t{data.cpu().flatten()[0].item()}\t{nccl_exponent}\t{bin(nccl_mantissa)} ({dtype_mapping[torch.bfloat16].mantissa_bits}) (e^{true_nccl_exponent}={2 ** true_nccl_exponent}) (e^{true_nccl_fraction}={2 ** true_nccl_fraction})")
                print("")
    except Exception:
        traceback.print_exc()
