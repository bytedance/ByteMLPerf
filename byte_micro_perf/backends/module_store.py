...
class IndexAddOp(torch.nn.Module):
    ...
    def process_inputs(self, input_tensor, source_tensor):
        index = torch.randint(0, input_tensor.shape[0], (source_tensor.shape[0],)).to(input_tensor.device)
...
class AllGatherOp(torch.nn.Module):
    ...
    def process_inputs(self, input_tensors):
        input_tensor_list = list(torch.chunk(input_tensors, dist.get_world_size(self.group)))
...
class AllToAllOp(torch.nn.Module):
    ...
    def process_inputs(self, input_tensor, output_tensor):
        ...
        output_tensor_list = list(torch.chunk(output_tensor, dist.get_world_size(self.group)))
