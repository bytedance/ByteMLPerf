...
class LlamaMLP(nn.Module):
    def __init__(self, config):
        ...
        self.gate_proj = nn.Linear(self.hidden_size, self.intermediate_size // self.mp_size, bias=False)
        self.up_proj = nn.Linear(self.hidden_size, self.intermediate_size // self.mp_size, bias=False)
        self.down_proj = nn.Linear(self.intermediate_size // self.mp_size, self.hidden_size, bias=False)
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        self.q_proj = nn.Linear(self.hidden_size, self.num_heads * self.head_dim // self.mp_size, bias=config.attention_bias)
        self.k_proj = nn.Linear(self.hidden_size, self.num_key_value_heads * self.head_dim // self.mp_size, bias=config.attention_bias)
        self.v_proj = nn.Linear(self.hidden_size, self.num_key_value_heads * self.head_dim // self.mp_size, bias=config.attention_bias)
        self.o_proj = nn.Linear(self.hidden_size // self.mp_size, self.hidden_size, bias=config.attention_bias)
        self._init_rope()
