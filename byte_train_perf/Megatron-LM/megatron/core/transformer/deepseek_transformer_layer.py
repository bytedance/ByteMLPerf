...
@dataclass
class DeepseekTransformerLayerSubmodules:
    """
...
class DeepseekTransformerLayer(MegatronModule, BaseTransformerLayer):
    ...
    def __init__(
        ...
        #NOTE [gqc] new feature: first_k_dense_layer
        if self.config.num_moe_experts is not None \
            and self.layer_number > self.config.first_k_dense_replace \
            and self.layer_number % self.config.moe_layer_freq == 0:
